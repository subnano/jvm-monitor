 ...
 * TODO
 *
 * - Need to handle stopped processes better
 ...
 * - alloc rate should be red when > 100
 * - CPU should be magenta when > 100%
 * - gc pause data should be scaled when grows
 *
 * @author Mark Wardell
 */
public class JTop implements EventRecorder<JvmEvent> {
...
//        }
    ...
    private void updateConsole(long timestamp) {
        ...
        while ((info = processInfos.get(row)) != null) {
            ...
            double cpuPercent = osProcess == null ? 0 : calculateCpuPercent(osProcess);
    ...
    private double calculateCpuPercent(OSProcess osProcess) {
